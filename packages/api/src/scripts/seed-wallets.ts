  ...
  for (const w of wallets) {
    ...
    await sql`
      INSERT INTO wallet_activity (
        ...
        ${sql.array(w.deployment_chains)}, ${w.deployment_calldata_bytes},
        ${sql.array(w.unique_protocols)}, ${sql.array(w.chains_active)},
        ${w.governance_votes}, ${sql.array(w.daos_participated)}, ${w.proposals_created}, ${w.delegation_events},
        ${w.bear_market_txs}, ${sql.array(w.active_month_set)}, ${sql.array(w.protocol_categories)},
        ${w.failed_transactions}, ${w.total_calldata_bytes},
        ${sql.array(w.recipient_addresses)}, ${sql.array(w.chain_protocol_pairs)}, ${sql.array(w.gas_price_set)}, ${sql.array(w.tx_hour_set)},
        ${w.create2_deployments}, ${sql.array(w.bear_market_periods)},
        ${w.execution_events}, ${sql.array(w.governance_chains)},
